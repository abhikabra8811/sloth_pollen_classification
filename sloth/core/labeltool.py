...
class LabelTool(QObject):
    ...
    def loadAnnotations(self, fname, handleErrors=True):
        ...
        #Abhishek Start: Set first image added as current image
        #return self._model._root.appendFileItem(fileitem)
        currentimage = self.currentImage()
        if currentimage is None:
            firstimage = self._model._root.childAt(0)
            self.setCurrentImage(firstimage)
        #Abhishek End: Set first image added as current image
