...
from PyQt4.QtGui import QWidget, QGroupBox, QVBoxLayout, QPushButton, QScrollArea, QLineEdit, QDoubleValidator, QIntValidator, QShortcut, QKeySequence, QCursor,\
                        QStandardItemModel, QStandardItem, QComboBox, QCompleter, QSortFilterProxyModel
import time
import logging
...
#         self.modelItem.setItem(int(v), 0, item)
#         #button.clicked.connect(bind(self.onButtonClicked, v))
#
#     def reset(self):
#         self._current_items = []
...
class PropertyEditor(QWidget):
    ...
    def onClassButtonPressed(self, label_class):
        if self._class_buttons[label_class].isChecked():
            ...
            #Abhishek End: Setting cross cursor for SceneView (image view)
        else:
    ...
    def startInsertionMode(self, label_class):
        ...
        #Label drop down
        #Abhishek
        idHandler = self.getHandler('id')
        if idHandler is not None:
            idHandler.ann_input_combo.setCurrentIndex(1)
...
#Abhishek : Class for autfill combo
