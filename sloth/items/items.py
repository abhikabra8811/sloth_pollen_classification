    ...
    def changeColor(self):
        if self._model_item is not None:
            ...
            if c is not None:
                self.setColor(c)
                return
        #Abhishek Start: Changing default color to red
...
class RectItem(BaseItem):
    ...
    def mouseMoveEvent(self, event):
        if self._resize:
            #Abhishek : set resize cursor : Fix me
            if self._resize_start_rect.bottom() or self._resize_start_rect.top():
                self.setCursor(Qt.SizeVerCursor)
            elif self._resize_start_rect.left() or self._resize_start_rect.right():
                self.setCursor(Qt.SizeHorCursor)
            elif self._resize_start_rect.topLeft() or self._resize_start_rect.bottomRight():
                self.setCursor(Qt.SizeFDiagCursor)
            elif self._resize_start_rect.topRight() or self._resize_start_rect.bottomLeft():
                self.setCursor(Qt.SizeBDiagCursor)
            #Abhishek : Set resize cursor
